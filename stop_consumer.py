...
logging.basicConfig(filename='consumer.log', format='%(asctime)s %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p', level=logging.INFO)
    ...
    try:
        while True:
            ...
            if (msg is None and total_count > 0):
                ...
                out_file = open(str(date.today())+"-stopeventoutput.json", "w")
